...
int Set(char *username, char *password) {
	...
	keychainItem[(__bridge id)kSecAttrService] = @"SecretStorage";
...
char *Get(char *username) {
	...
	if(sts == noErr) {
		...
		NSLog(@"ios keychain Get: Error Code: %d", (int)sts);
	}
...
func init() {
	defaultProvider = iosProvider{}
}

func (p iosProvider) Get(Service, Username string) (string, error) {
	res := C.GoString(C.Get(C.CString(fmt.Sprintf("%s::%s", Service, Username))))
	if len(res) == 0 {
		return "", ErrNotFound
	}
...
func (p iosProvider) Set(Service, Username, Password string) error {
	ret := C.Set(C.CString(fmt.Sprintf("%s::%s", Service, Username)), C.CString(Password))
	switch ret {
	case errSecSuccess:
		return nil
